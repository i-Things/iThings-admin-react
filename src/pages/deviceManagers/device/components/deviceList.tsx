import { CreateForm } from '@/pages/deviceManagers/device/components/createForm';
    ...
    },
    {
      ...
      render: (text, record: DEVICE_INFO) => [
        <a
          ...
          onClick={() => {
            history.push(
              '/deviceManagers/device/detail/' + record.productID + '/' + record.deviceName + '/1',
            );
          }}
        >
          {text}
        ...
        <>
          ...
          {deviceType[record.productID as string]?.text === '网关' && (
            <div>
              <a
                className=""
                href={`/deviceManagers/device/detail/${record.productID}/${record.deviceName}/7`}
              >
                子设备管理
              </a>
            </div>
