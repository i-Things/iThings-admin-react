...
const data = [{
    title: '设备触发',
    key: TriggerWayType.device,
    desc: '适用于设备数据或行为满足触发条件时，执行指定的动作',
    imgSrc: deviceImg
}, {
    title: '手动触发',
    key: TriggerWayType.manual,
    desc: '适用于第三方平台向物联网平台下发指令控制设备.',
    imgSrc: manualImg
}, {
    title: '定时触发',
    key: TriggerWayType.timer,
    desc: '适用于定期执行固定任务.',
    imgSrc: timingImg
}]

const TriggerType: React.FC<triggerTypeProps> = (props) => {
    const { isChecked = TriggerWayType.manual, onCheck } = props;

    const [activeKey, setActiveKey] = useState(TriggerWayType.device)
