...
const CardItem: React.FC<ruleSceneList & ruleSceneProps> = (props) => {
  ...
  return (
    <div className={styles['card-item-wrap']}>
      {list.map((item) => {
        return (
          <div className={styles['box-wrap']} key={item.id}>
            <div
            ...
            >
              <div className={styles['top-icon-wrap']}>
                <div className={styles['top-icon']}>{TriggerWayType[item.triggerType]}</div>
              </div>
              ...
              <div className={styles['right-status-wrap']}>
                <div className={styles['right-status']}>{item?.state === 1 ? '启用' : '禁用'}</div>
              </div>
            </div>
