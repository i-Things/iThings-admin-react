...
const IndexPage = () => {
  ...
  const Tools = (record: API.scene): React.ReactNode => {
    return <div className='card-item-bottom-wrap'>
      <div className='bottom-item-wrap'>
        <Button className={classNames('btn-flex', 'btn', 'mr-8')} type='link'
          onClick={() => console.log('record', record
          )}>编辑</Button>
        <Button className={classNames('btn-flex', 'btn', 'mr-8')} type='link'>启用</Button>
        <Button className={classNames('btn-side', 'btn')} type='link'>删除</Button>
      </div>
    </div>
  }
  ...
  return (
    <PageContainer>
      <Card>
        <CardItem
          toolRender={(record) => (Tools(record))}
          list={ruleSceneList?.data?.list}
        />
      </Card>
    </PageContainer>
  );
